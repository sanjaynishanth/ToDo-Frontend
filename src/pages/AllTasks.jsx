...
const AllTasks = () => {
    ...
    const handleToggleComplete = async (taskId, currentStatus) => {
        ...
        try {
            const res = await fetch(`https://todo-backend-e14k.onrender.com/api/tasks/${taskId}`, {
                method: 'PUT',
                headers: {
    ...
    const handleEditSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch(`https://todo-backend-e14k.onrender.com/api/tasks/${editingTaskId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
    ...
    const handleShareTask = async () => {
        try {
            const res = await fetch(`https://todo-backend-e14k.onrender.com/api/tasks/${shareTaskId}/share`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
            ...
            if (res.ok) {
                ...
                fetchTasks(); // Re-fetch to update sharedWith list
                setShareTo('');
            } else {
